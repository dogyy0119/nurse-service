<template>
	<view class="container">
		...
		<view class="grid-wrapper"
			:style="'grid-template-columns: repeat(' + columns + ', 1fr); gap:' + gapNumber + 'rpx;'">
			<view v-for="(item, index) in cityArray" :key="index" :class="{'grid-item': true}" @click="tapGrid(index)"
				:style="{height: height + 'rpx'}">
				<image class="icon-house" src="/static/images/img_11.png" :alt="item.name" />
		...
		<view class="order-instructions">
			...
			<view class="instructions-content">
				<p>1. 预约请仔细核对联系方式地址是否正确。2. 护士接单后一般会在30分钟内跟您电话联系，如遇到没有及时联系的请联系护士或者平台客服，以免耽误您的行程安排。3.
					预约订单护士接单后由于您个人原因退款将收取百分之30的手续费；护士接单已经出发后由于个人原因取消订单将扣除百分之50的手续费；护士未接单取消订单全额退款。4.
					晚上超过8点的订单统一加收80的夜班费。5. 为保障您的权益，请在护士完成订单后再确认服务完成。</p>
			</view>
		</view>
...
<style scoped>
	.container {
		...
		position: relative;
		/* 或者 absolute */
		background-image: -webkit-linear-gradient(-77deg, #DBE9FA 0%, #DBE9FA 100%);
